///////////////////////////////////////////////////////////
//  CustomerNotification.cs
//  Implementation of the Class CustomerNotification
//  Generated by Enterprise Architect
//  Created on:      15-Jun-2024 10:33:04 AM
//  Original author: T. Kostic
///////////////////////////////////////////////////////////

using System;
using System.Collections.Generic;
using System.Text;
using System.IO;



using TC57CIM.IEC61970.Base.Domain;
using TC57CIM.IEC61968.Customers;
namespace TC57CIM.IEC61968.Customers {
	/// <summary>
	/// Conditions for notifying the customer about the changes in the status of their
	/// service (e.g., outage restore, estimated restoration time, tariff or service
	/// level change, etc.)
	/// </summary>
	public class CustomerNotification {

		/// <summary>
		/// Type of contact (e.g., phone, email, etc.).
		/// </summary>
		public TC57CIM.IEC61970.Base.Domain.String contactType;
		/// <summary>
		/// Value of contact type (e.g., phone number, email address, etc.).
		/// </summary>
		public TC57CIM.IEC61970.Base.Domain.String contactValue;
		/// <summary>
		/// Earliest date time to call the customer.
		/// </summary>
		public TC57CIM.IEC61970.Base.Domain.DateTime earliestDateTimeToCall;
		/// <summary>
		/// Latest date time to call the customer.
		/// </summary>
		public TC57CIM.IEC61970.Base.Domain.DateTime latestDateTimeToCall;
		/// <summary>
		/// Trigger for this notification.
		/// </summary>
		public NotificationTriggerKind trigger;
		/// <summary>
		/// Customer requiring this notification.
		/// </summary>
		public TC57CIM.IEC61968.Customers.Customer Customer;
		/// <summary>
		/// All trouble tickets with this notification.
		/// </summary>
		public TC57CIM.IEC61968.Customers.TroubleTicket TroubleTickets;

		public CustomerNotification(){

		}

		~CustomerNotification(){

		}

		public virtual void Dispose(){

		}

	}//end CustomerNotification

}//end namespace Customers