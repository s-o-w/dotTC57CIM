///////////////////////////////////////////////////////////
//  PanPricingDetail.cs
//  Implementation of the Class PanPricingDetail
//  Generated by Enterprise Architect
//  Created on:      15-Jun-2024 10:33:19 AM
//  Original author: T. Kostic
///////////////////////////////////////////////////////////

using System;
using System.Collections.Generic;
using System.Text;
using System.IO;



using TC57CIM.IEC61970.Base.Domain;
namespace TC57CIM.IEC61968.Metering {
	/// <summary>
	/// Detail for a single price command/action.
	/// </summary>
	public class PanPricingDetail {

		/// <summary>
		/// Alternative measure of the cost of the energy consumed. An example might be the
		/// emissions of CO2 for each kWh of electricity consumed providing a measure of
		/// the environmental cost.
		/// </summary>
		public TC57CIM.IEC61970.Base.Domain.Float alternateCostDelivered;
		/// <summary>
		/// Cost unit for the alternate cost delivered field. One example is kg of CO2 per
		/// unit of measure.
		/// </summary>
		public TC57CIM.IEC61970.Base.Domain.String alternateCostUnit;
		/// <summary>
		/// Current time as determined by a PAN device.
		/// </summary>
		public TC57CIM.IEC61970.Base.Domain.DateTime currentTimeDate;
		/// <summary>
		/// Price of the commodity measured in base unit of currency per 'unitOfMeasure'.
		/// </summary>
		public TC57CIM.IEC61970.Base.Domain.Money generationPrice;
		/// <summary>
		/// Ratio of 'generationPrice' to the "normal" price chosen by the commodity
		/// provider.
		/// </summary>
		public TC57CIM.IEC61970.Base.Domain.Float generationPriceRatio;
		/// <summary>
		/// Price of the commodity measured in base unit of currency per 'unitOfMeasure'.
		/// </summary>
		public TC57CIM.IEC61970.Base.Domain.Money price;
		/// <summary>
		/// Ratio of 'price' to the "normal" price chosen by the commodity provider.
		/// </summary>
		public TC57CIM.IEC61970.Base.Domain.Float priceRatio;
		/// <summary>
		/// Pricing tier as chosen by the commodity provider.
		/// </summary>
		public TC57CIM.IEC61970.Base.Domain.Integer priceTier;
		/// <summary>
		/// Maximum number of price tiers available.
		/// </summary>
		public TC57CIM.IEC61970.Base.Domain.Integer priceTierCount;
		/// <summary>
		/// Label for price tier.
		/// </summary>
		public TC57CIM.IEC61970.Base.Domain.String priceTierLabel;
		/// <summary>
		/// Label of the current billing rate specified by commodity provider.
		/// </summary>
		public TC57CIM.IEC61970.Base.Domain.String rateLabel;
		/// <summary>
		/// Register tier accumulating usage information.
		/// </summary>
		public TC57CIM.IEC61970.Base.Domain.String registerTier;
		/// <summary>
		/// Defines commodity as well as its base unit of measure.
		/// </summary>
		public TC57CIM.IEC61970.Base.Domain.String unitOfMeasure;

		public PanPricingDetail(){

		}

		~PanPricingDetail(){

		}

		public virtual void Dispose(){

		}

	}//end PanPricingDetail

}//end namespace Metering